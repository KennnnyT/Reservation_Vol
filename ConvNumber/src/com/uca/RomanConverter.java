/*
	Noms : TSHIBUABUA & RASANDISON
	Prenoms : Kenny & Marc
*/

package com.uca;


import java.util.Collection;
import java.util.ArrayList;
import java.util.regex.Pattern;
import java.util.regex.Matcher;
import java.lang.Exception;


public class RomanConverter{
	
	// Table des symboles
	private static final Collection<RomanNumber> SYMBOLS = new ArrayList<>();
	static {
		SYMBOLS.add(new RomanNumber(1000, "M"));
		SYMBOLS.add(new RomanNumber(900, "CM"));
		SYMBOLS.add(new RomanNumber(500, "D"));
		SYMBOLS.add(new RomanNumber(400, "CD"));
		SYMBOLS.add(new RomanNumber(100, "C"));
		SYMBOLS.add(new RomanNumber(90, "XC"));
		SYMBOLS.add(new RomanNumber(50, "L"));
		SYMBOLS.add(new RomanNumber(40, "XL"));
		SYMBOLS.add(new RomanNumber(10, "X"));
		SYMBOLS.add(new RomanNumber(9, "IX"));
		SYMBOLS.add(new RomanNumber(5, "V"));
		SYMBOLS.add(new RomanNumber(4, "IV"));
		SYMBOLS.add(new RomanNumber(1, "I"));
	  }

	// Expression reguliere de validation
	// on definis des pattern a respecter 

	private static final Pattern VALIDATION_RE = Pattern.compile("^M{0,3}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})$");



	public static String getRomanFromNumber(int nombre) throws IllegalArgumentException{
		String resultat = "";

		if ( nombre <= 0 || nombre >4000 )
		{
			throw new IllegalArgumentException("La valeur passée en parametre est negative ou soit superieur à 4000 ");

		}

		else
		{

			for(RomanNumber i: SYMBOLS)
			{
				while(i.getValue() <= nombre)
				{
					resultat += i.getRoman();
					nombre -= i.getValue();
				}
			}
			return resultat;
		}
	}



	
	public static int getNumberFromRoman(String nombre) throws IllegalArgumentException{

		Matcher matcher = VALIDATION_RE.matcher(nombre);
		boolean matchfound = matcher.find();
		int resultat = 0;
		int index = 0;

		for(RomanNumber i: SYMBOLS){
			while(((index + i.getRoman().length()) <= nombre.length()) && (nombre.substring(index,index + i.getRoman().length()).equals(i.getRoman()))){
				resultat += i.getValue();
				index += i.getRoman().length();
			}
		}
		return resultat;
	}
}